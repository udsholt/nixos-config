...
{
	# Install php and deps (note that redis is a custom package until it hits unstable)
	...
	services.lighttpd = {
		...
		enableModules = [ "mod_rewrite" "mod_evhost" "mod_accesslog" "mod_setenv" "mod_fastcgi" ];
		extraConfig = ''
			fastcgi.server += ( 
				".php" => (
			...
			$HTTP["host"] =~ "\.flex.aws.peytz.dk" {
	            index-file.names = ("index.php")
	            evhost.path-pattern = "/srv/www/userhub/web/" 
	            accesslog.filename  = "/srv/www/access.log"
	            server.reject-expect-100-with-417 = "disable"
	            setenv.add-environment = (
	                "PEYTZ_ENV" => "dev",
